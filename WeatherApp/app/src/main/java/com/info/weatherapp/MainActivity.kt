...
import com.info.weatherapp.adapter.RvAdapter
import com.info.weatherapp.data.forecastModels.ForecastData
import com.info.weatherapp.databinding.ActivityMainBinding
import com.info.weatherapp.databinding.BottomSheetLayoutBinding
import com.info.weatherapp.utils.RetrofitInstance
...
class MainActivity : AppCompatActivity() {
    ...
    private fun getForecast() {
        GlobalScope.launch(Dispatchers.IO) {
            val response =  try {
                RetrofitInstance.api.getForecast(city,"metric",applicationContext.getString(R.string.api_key))
            }catch (e:IOException){
               ...
               withContext(Dispatchers.Main){
                   ...
                   binding.apply {
                       ...
                       tvUpdateTime.text= "Last Update: ${
                          dateFormatConverter(data.dt!!.toLong())
                       }"
    ...
    private fun getPollution(lat: Double, lon: Double) {
        GlobalScope.launch(Dispatchers.IO) {
            val response =  try {
                RetrofitInstance.api.getPollution(lat,lon,"metric",applicationContext.getString(R.string.api_key))
            }catch (e:IOException){
                Toast.makeText(applicationContext, "app error ${e.message}", Toast.LENGTH_SHORT).show()
                return@launch
            }catch (e:HttpException){
